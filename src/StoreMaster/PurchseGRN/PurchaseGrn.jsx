...
const PurchaseGrn = () => {
  ...
  useEffect(() => {
    const fetchSupplierState = async () => {
      if (formData.Supp_Cust) {
        try {
          const res = await axios.get("https://sellerp-backend.onrender.com/All_Masters/api/SupplierData/");
          const sup = res.data.find((s) => s.Name === formData.Supp_Cust);
          if (sup && sup.State_Code) {
            setSupplierState(sup.State_Code);
          } else {
            setSupplierState("");
          }
        } catch (err) {
      ...
      try {
        ...
        if (items && Array.isArray(items)) {
          const filtered = items.filter(
            (item) =>
              (item && item.Item && item.Item.toLowerCase().includes(value.toLowerCase())) ||
              (item && item.ItemDescription && item.ItemDescription.toLowerCase().includes(value.toLowerCase()))
          );
  ...
  const handleAddItem = async (itemName) => {
    try {
      ...
      const items = await getPurchaseGrnItems();
      if (!items || !Array.isArray(items)) {
        alert("Failed to load items or PO is not selected.");
        return;
      }
      ...
      // FETCH GST DETAILS
      try {
        const itemFieldsRes = await axios.get(`https://sellerp-backend.onrender.com/All_Masters/Fetch_Item_fields/?q=${encodeURIComponent(itemName)}`);
        if (itemFieldsRes.data && itemFieldsRes.data.length > 0) {
          const fullItemData = itemFieldsRes.data[0];
          if (fullItemData) {
            const tax = fullItemData.taxdetails || {};
            ...
            const qty = parseFloat(initializedNewItem.GrnQty || 0);
            const assValue = rate * qty;
            
            const cgstAmt = (assValue * parseFloat(cgst)) / 100;
            const sgstAmt = (assValue * parseFloat(sgst)) / 100;
            const igstAmt = (assValue * parseFloat(igst)) / 100;
            const utgstAmt = (assValue * parseFloat(utgst)) / 100;
            const totalSgstOrUtgst = parseFloat(sgst) > 0 ? sgstAmt : utgstAmt;

            const newGst = {
              ItemCode: itemName,
              HSN: fullItemData.HSN_SAC_Code || fullItemData.SAC || "",
              Rate: newItem.Rate || "0",
              DiscRate: newItem.Disc || "0.00",
              Qty: initializedNewItem.GrnQty || "0",
              Discount: "0.00",
              Packing: "0.00",
              Transport: "0.00",
              AssValue: assValue.toFixed(2),
              CGST: cgstAmt.toFixed(2),
              SGST: totalSgstOrUtgst.toFixed(2),
              IGST: igstAmt.toFixed(2),
              Vat: "0.00",
              Cess: "0.00",
              SubTotal: assValue.toFixed(2),
              Total: (assValue + cgstAmt + totalSgstOrUtgst + igstAmt).toFixed(2),
              ToolAmort: "0.00"
            };

            setGstDetails((prev) => [...prev, newGst]);
            setOriginalGstDetails((prev) => [...prev, JSON.parse(JSON.stringify(newGst))]);
          }
        }
      } catch (err) {
        console.error("Error fetching GST info for item:", err);
      }
    ...
    try {
      const payload = {
        ...
        GrnGst: gstDetails.map((gst) => ({
          ...
          PackAmt: gst.Packing || null, 
          TransAmt: gst.Transport || null, 
...
<h5 className="header-title text-start" style={{ fontWeight: 800, fontSize: '1.8rem', background: 'linear-gradient(90deg, #2563eb, #4f46e5)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: '16px' }}>{isEditMode ? "Edit Purchase GRN" : "New Purchase GRN"}</h5>
                ...
                <div className="Purchasegrn-main mt-2" style={{ maxWidth: "100%" }}>
                              ...
                              >
                                {suggestions.map((item) => (
                                  <li
                                    ...
                                    style={{ cursor: "pointer", fontSize: "0.9rem", whiteSpace: "normal", wordWrap: "break-word" }}
                                  >
                                ...
                                <tr key={item.id}>
                                  ...
                                  <td>
                                    <input
                                      ...
                                      style={{ width: "100%", minWidth: "50px", maxWidth: "80px", padding: "2px 4px", fontSize: "11px" }}
                                      value={item.GrnQty || ""}
                                      onChange={(e) => {
                                  ...
                                  <td>
                                    <input
                                      type="text"
                                      className="form-control form-control-sm"
                                      style={{ width: "100%", minWidth: "60px", maxWidth: "90px", padding: "2px 4px", fontSize: "11px" }}
                                      value={item.HeatNo || ""}
                                      onChange={(e) => {
