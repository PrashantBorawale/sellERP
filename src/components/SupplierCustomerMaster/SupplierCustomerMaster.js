    ...
    if (validate()) {
      try {
        ...
        if (response && response.status === false) {
          ...
          try {
            const localDataStr = localStorage.getItem("localSuppliersData");
            const localData = localDataStr ? JSON.parse(localDataStr) : [];
            // Merge dataToSubmit with the full backend response to capture generated fields (e.g., 'number', 'id')
            const recordToSave = { ...dataToSubmit, ...response, id: isEditMode ? id : (response.id || Date.now()) };
            if (isEditMode) {
              const existingIdx = localData.findIndex(item => item.id === id);
              if (existingIdx >= 0) {
                localData[existingIdx] = recordToSave;
              } else {
                localData.push(recordToSave);
              }
            } else {
              localData.push(recordToSave);
            }
            
            localStorage.setItem("localSuppliersData", JSON.stringify(localData));
          } catch (e) {
