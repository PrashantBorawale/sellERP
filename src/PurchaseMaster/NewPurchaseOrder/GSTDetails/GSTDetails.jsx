...
const GSTDetails = ({ updateFormData = () => {}, itemDetails = [], supplierState }) => {
  ...
  useEffect(() => {
    const calculatedGSTDetails = itemDetails.map((item) => {
      ...
      // Since we already calculated the state-specific GST logic in ItemDetails and saved it 
      // directly to item.GST_Details.CGST, item.GST_Details.SGST, etc., we can just use them!
      const cgstRate = Number(item.CGST || (item.GST_Details ? item.GST_Details.CGST : 0)) || 0
      const sgstRate = Number(item.SGST || (item.GST_Details ? item.GST_Details.SGST : 0)) || 0
      const igstRate = Number(item.IGST || (item.GST_Details ? item.GST_Details.IGST : 0)) || 0
      // If the UI has UTGST, you would handle it similarly. Assuming SGST maps to UTGST visually if UTGST is set
      const utgstRate = Number(item.UTGST || (item.GST_Details ? item.GST_Details.UTGST : 0)) || 0

      // If utgst is present, it usually takes the place of SGST in this table format:
      const effectiveSgstRate = sgstRate > 0 ? sgstRate : utgstRate;

      const cgstAmount = (assessableValue * cgstRate) / 100
      const sgstAmount = (assessableValue * effectiveSgstRate) / 100
