...
const NewJobworkPurchase = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (mountedRef.current) {
          ...
          if (result && result.length > 0 && supplierName) setShowDropdown(true);
        }
      } catch (err) {
  ...
  useEffect(() => {
    const fetchSupplierStateRules = async () => {
      ...
      try {
        ...
        const suppNameOnly = supplierName.split('|')[0].trim().toLowerCase();
        const supplierInfo = data.find(s => (s.Name || "").toLowerCase() === suppNameOnly);
        
        if (supplierInfo) {
          const stateCode = supplierInfo.State_Code || "";
          if (mountedRef.current) {
            setSupplierState(stateCode);
          }
        }
      } catch (err) {
        console.error("Error fetching supplier state rules:", err);
      }
    };

    fetchSupplierStateRules();
  }, [supplierName]);
        ...
        Item_Detail_Enter:
          allTabsData.itemDetails && allTabsData.itemDetails.length > 0
            ? allTabsData.itemDetails.map((item) => ({
              ...
              item_type: item.item_type || item.SelectItem || "",
        ...
        Gst_Details:
          allTabsData.gstDetails && allTabsData.gstDetails.length > 0
            ? allTabsData.gstDetails.map((item) => ({
              ...
              Discount_Amt: item.DiscountAmt || "0",
              Packing: item.Packing || "0",
              Transport: item.Transport || "0",
              AssValue: item.AssValue || "0",
              CGST: item.CGST || "0",
              CGST_Amt: item.CGSTAmt || "0",
              SGST: item.SGST || "0",
              SGST_Amt: item.SGSTAmt || "0",
              IGST: item.IGST || "0",
              IGST_Amt: item.IGSTAmt || "0",
              UTGST: item.UTGST || "0",
              UTGST_Amt: item.UTGSTAmt || "0",
              Total: item.Total || "0",
            }))
            : [],
                      ...
                      <div className="row g-3 align-items-end">
                        ...
                        <div className="col-md-4 text-start position-relative">
                          ...
                          {showDropdown && dropdownData && dropdownData.length > 0 && (
                            <ul className="dropdown-menu show position-absolute shadow w-100 mt-1" style={{ top: '100%', left: 0, zIndex: 1050, maxHeight: "180px", overflowY: 'auto', padding: '0.25rem 0' }}>
                              {dropdownData.map((supplier) => (
                                <li key={supplier.number} className="dropdown-item py-1 border-bottom" onClick={() => handleSelectSupplier(supplier)} style={{ cursor: 'pointer', whiteSpace: 'normal', wordWrap: 'break-word', padding: '4px 8px' }}>
                                  <div className="fw-bold" style={{ fontSize: "11px" }}>{supplier.Name}</div>
                                  <div className="text-muted" style={{ fontSize: "10px" }}>{supplier.number}</div>
                                </li>
                              ))}
                            </ul>
                          )}
